<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');

class Division extends MY_Controller {

	var $title = 'Division';
	var $url = 'division';
	var $db_table = 'division';
	
	public function __construct()
	{
		parent::__construct();
		check_admin_login('redirect');
		$this->load->model('model_division');
	}	
	
	public function index()
	{
		$data = array(
			'title'	=> 'List ' . $this->title,
			'css'	=> array('alertify.core', 'alertify.bootstrap', 'jquery.dataTables'),
			'js'	=> array('alertify', 'jquery.dataTables.min', 'admin/list')
		);

		$db_query = $this->model_division->list_data();
		
		$data['result'] = $db_query;

		$this->show('admin/' . $this->url . '/list_' . $this->url, $data);
	}
	
	public function add()
	{
		$data = array(
			'title'	=> 'Add ' . $this->title,
			'css'	=> array(),
			'js'	=> array('admin/form')
		);
		
		$data['admin_list'] = $this->model_division->get_admin();
		
		$this->form_validation->set_rules('division_name', 'division name', 'trim|required');
		$this->form_validation->set_rules('division_head', 'division head', 'required');
		$this->form_validation->set_rules('flag', 'flag', 'required');
		$this->form_validation->set_rules('memo', 'memo', 'trim');
		
		if ($this->form_validation->run() == FALSE)
		{
			$this->show('admin/' . $this->url . '/add_' . $this->url, $data);
		}
		else
		{
			$this->model_division->insert();
			redirect(base_url('admin/' . $this->url));
		}
	}
	
	function view($unique_id)
	{
		$data = array(
			'title'	=> 'View ' . $this->title,
			'css'	=> array('jquery.fancybox'),
			'js'	=> array('admin/form')
		);
		
		$data['row'] = $this->model_division->get($unique_id);
		
		if ( ! $data['row']) redirect(base_url('admin/' . $this->url));
		
		$data['admin_list'] = $this->model_division->get_admin();
		
		$this->form_validation->set_rules('division_name', 'division name', 'trim|required');
		$this->form_validation->set_rules('division_head', 'division head', 'required');
		$this->form_validation->set_rules('flag', 'flag', 'required');
		$this->form_validation->set_rules('memo', 'memo', 'trim');
		
		if ($this->form_validation->run() == FALSE)
		{
			$this->show('admin/' . $this->url . '/view_' . $this->url, $data);
		}
		else
		{
			$this->model_division->update($unique_id);
			redirect(base_url('admin/' . $this->url));
		}
	}
}


/* End of file division.php */
/* Location: ./application/controllers/admin/division.php */